...
def main():
    root = tk.Tk()
    root.title("Apex Legends Current Map")

    label_font = ('Tw Cen MT', 20)

    label_map = tk.Label(root, text="Map: ", font=label_font)
    label_time_range = tk.Label(root, text="Time Range: ", font=label_font)
    label_time_remaining = tk.Label(root, text="Time Remaining: ", font=label_font)

    label_map.pack()
    label_time_range.pack()
    ...
    while True:
        current_time_str = dt.now(pytz.timezone('America/New_York')).strftime("%Y-%m-%d %H:%M:%S %z")
        map_data = map_at_time(current_time_str)

        label_map.config(text=f"Map: {map_data['map']}")
        label_time_range.config(text=f"{map_data['start']} - {map_data['end']}")
        label_time_remaining.config(text=f"Time Remaining: {map_data['remaining']}")
